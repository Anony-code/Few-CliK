...
class Trainer:
    def __init__(self, data_loaders, dataset, parameter):
        ...
        self.train_data_loader_target = data_loaders[3]
        self.dev_data_loader_target = data_loaders[4]
        # parameters
        ...
        self.metaR_target = MetaR(dataset, parameter)
        self.metaR.to(self.device)
        self.metaR_target.to(self.device)
        # optimizer
        self.optimizer = torch.optim.Adam(self.metaR.parameters(), self.learning_rate)
        self.optimizer_target = torch.optim.Adam(self.metaR_target.parameters(), self.learning_rate)
    ...
    def reload(self):
        ...
        if os.path.isfile(state_dict_file):
            self.metaR_target.load_state_dict(state)
        else:
    ...
    def cal_pn_score(self, task, iseval=False, curr_rel=''):
        p_score, n_score = self.metaR(task, iseval, curr_rel)
        return p_score, n_score

    def KL(self, task, p, n, iseval=False, curr_rel=''):
        p_score, n_score = self.cal_pn_score(task, iseval, curr_rel)
        pn = torch.cat((p, n), 1)
        pn_ = torch.cat((p_score, n_score), 1)
        kl_loss = torch.mean(torch.norm(pn_ - pn, 2, -1))

        ##
        y = torch.Tensor([1]).to(self.device)
        loss_cont_left = self.metaR_target.loss_func(p_score, n, y)
        loss_cont_right = self.metaR_target.loss_func(p, n_score, y)

        return kl_loss, loss_cont_left, loss_cont_right
    ...
    def do_one_step_target(self, task, iseval=False, curr_rel=''):
        loss, p_score, n_score, kl_loss, cont_loss = 0, 0, 0, 0, 0
        if not iseval:
            self.optimizer_target.zero_grad()
            p_score, n_score = self.metaR_target(task, iseval, curr_rel)
            kl_loss, cont_loss_left, cont_loss_right = self.KL(task, p_score, n_score, iseval=False, curr_rel=curr_rel)
            cont_loss = cont_loss_left + cont_loss_right
            y = torch.Tensor([1]).to(self.device)
            loss = self.metaR_target.loss_func(p_score, n_score, y)
            loss_ = loss + 0.01 * kl_loss + 2.0 * cont_loss
            loss_.backward()
            ...
            y = torch.Tensor([1]).to(self.device)
            loss = self.metaR_target.loss_func(p_score, n_score, y)
        return loss, p_score, n_score, kl_loss, cont_loss
    ...
    def train(self):
        ...
        for e in range(self.epoch+1):
            ...
            if e % self.eval_epoch == 0 and e != 0:
                ...
                valid_data = self.eval(istest=False, istarget=False, epoch=e)
        ...
        print('Training-Source has finished')
    ...
    def train_target(self):
        ...
        for e in range(self.epoch+1, self.epoch*5+1):
            # sample one batch from data_loader
            train_task, curr_rel = self.train_data_loader_target.next_batch()
            loss, _, _, kl_loss, cont_loss = self.do_one_step_target(train_task, iseval=False, curr_rel=curr_rel)
            # print the loss on specific epoch
            if e % self.print_epoch == 0:
                loss_num = loss.item()
                self.write_training_log({'Loss': loss_num}, e)
                print("Epoch: {}\tLoss: {:.4f}\tKL Loss: {:.4f}\tContrasitve Loss: {:.4f}".format(e, loss_num, kl_loss, cont_loss))
                # do evaluation on specific epoch
            if e % self.eval_epoch == 0 and e != 0:
                print('Epoch  {} has finished, validating...'.format(e))

                print('---------------- dev phase start-----------------\n')
                valid_data = self.eval(istest=False, istarget=True, epoch=e)
                self.write_validating_log(valid_data, e)
                ...
                # test_data = self.eval(istest=True, istarget=True, epoch=e)
                # print('---------------- test phase end-----------------\n')

        print('Training-Target has finished')
        print('\tBest epoch is {0} | {1} of valid set is {2:.3f}'.format(best_epoch, metric, best_value))
        self.save_best_state_dict(best_epoch)
        print('Finish')
    ...
    def eval(self, istest=False, istarget=False, epoch=None):

        if istarget:
            self.metaR_target.eval()
            self.metaR_target.rel_q_sharing = dict()
        else:
            self.metaR.eval()
            # clear sharing rel_q
        ...
        if istest:
            ...
            data_loader = self.dev_data_loader_target
        else:
        ...
        while True:
            ...
            if istarget:
                _, p_score, n_score, _, _ = self.do_one_step_target(eval_task, iseval=True, curr_rel=curr_rel)
            else:
